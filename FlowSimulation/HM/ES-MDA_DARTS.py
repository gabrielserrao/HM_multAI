...
def RunModels(destDir, MGrid, MScalar,Ne):
    for j,mGridColumn in enumerate(MGrid.T):
        ...
        mGridColumn.to_pickle(f'{job_file}.pkl')
    ...
    #filename = f'/tudelft.net/staff-umbrella/gabrielserrao/FlowSimulation/HM/data/simulations/it{3}/data_model'+str(Ne-1)+'.pkl'
...
#delete all data in /data/simulations/it{i} folders

# for i in range(0, 3):
#     destDir = f'{os.getcwd()}/data/simulations/it{i}'
...
for alpha in alphas:
    ...
    RunModels(destDir, MGrid, MScalar, Ne)
    time.sleep(60)
